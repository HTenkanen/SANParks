...
def createWordCloud(mask_filepath, df, label_col, label_name, text_name, outputname, suffix):
    ...
    if len(posts) > 0:
        ...
        if len(text) > 0:
            ...
            name = "Twitter_%s_%s_%s.png" % (outputname, year, suffix)
...
some_paths = [r"Addo_NP_Twitter_2012-2015.shp",
              r"iSimangaliso_NP_Twitter_2012-2015.shp",
              r"Kruger_NP_Twitter_2012-2015.shp",
              r"TableMountain_NP_Twitter_2012-2015.shp"
              ]

# File paths
boundary_root = r"P:\h510\some\data\south-africa\gis_layers\national_park_regions"
some_root = r"P:\h510\some\data\south-africa\social_media\twitter\Shapefiles\Selected"

mask_dir = r"P:\h510\some\data\south-africa\images\national_park_png"
wordcloud_dir = r"P:\h510\some\figures\Wordclouds\Twitter\2012-2015\100Words"
...
# Label ==> Entire parks
# 
area_identifier = 'Label' #"REGION" #"SECTION" #"Label"
...
SUFFIX = '100Words_removedStopwords'
...
for idx, boundary in enumerate(boundaries_list):
    ...
    some = some.sort_values(by='created_at')
    some = some.reset_index(drop=True)
    some['created_at'] = some['created_at'].astype(str)
    # Strip Timezone information out
    some['created_at'] = some['created_at'].str.slice(0, 18)
    some['time'] = pd.to_datetime(some['created_at'])
    ...
    if iterate_areas:
        ...
        # Create Wordcloud
        posts = createWordCloud(mask_filepath=mask_fp, label_col=area_identifier, df=some, label_name=label, outputname=label, text_name='text', suffix=SUFFIX)
